...
def red_team_actions(config, team_type):
    ...
    if team_type == 'dynamic':
        read_path = Path(__file__).parents[
            1] / 'recreate/config/red_team_config_dynamic.yml'
    elif team_type == 'static':
        read_path = Path(
            __file__).parents[1] / 'recreate/config/red_team_config_static.yml'
    else:
        read_path = Path(__file__).parents[
            1] / 'recreate/config/red_team_config_baseline.yml'
    ...
    for i in range(config['simulation']['n_uav_platoons']):
        ...
        init_nodes_index = attr['uav_platoon']['initial_nodes_pos'][i]
        actions_uav['centroid_pos'] = nodes[init_nodes_index]['position']
    ...
    for i in range(config['simulation']['n_ugv_platoons']):
        ...
        # Update the centroid position
        init_nodes_index = attr['ugv_platoon']['initial_nodes_pos'][i]
        actions_ugv['centroid_pos'] = nodes[init_nodes_index]['position']
